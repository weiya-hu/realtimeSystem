						...
						<div class="diged">
							...
							<div class="rightt" style="width: 55%;">
								<div class="gas flexl" v-for="(item,index) in gasdata.in.left.ec">
									<div class="gasname fleximg">
										<img src="../assets/person.png"/>
									</div>
									<div class="typenamehv">
						...
						<div class="diged">
							...
							<div class="rightt" style="width: 30%;">
								<div class="gas flexl" v-for="(item,index) in gasdata.out.left.dk">
									<div class="gasname"><img src="../assets/person.png"/></div>
									<div class="typenamehv">
										<div>设备名称：{{item.name}}</div>
										<div>位置：{{item.address}}</div>
									</div>
	...
	export default {
		...
		methods: {
			...
			getnormlist() {
				...
				this.$http.post(that.global.domainurl + '/jeecg-boot/replace/getBidsectionListByTypeAndBelong', {
					...
					if(res.data.code === 200) {
						...
						localStorage.setItem("listnormPerson", JSON.stringify(arr));
						return arr
					}
			...
			getGasData(id){
				...
				this.$http.post(that.global.domainurl + '/jeecg-boot/replace/findPersonSize', {
					bidsectionId:id
				}).then(res => {
					...
					if(res.data.code === 0) {
						...
						console.log(that.gasdata.in.left)
					}
				}).catch(function(error) {
					that.fullscreenLoading = false;
					console.log(error)
